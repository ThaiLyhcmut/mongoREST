...
import {
  ...
  ChainedOperations,
  ScriptParserOptions
} from '../config/core/script-parser.config';
import { ParsedScript } from '../config/middleware/script-pasing.config';

class MongoScriptParser {
  private operators: Set<string>;
  private options: ScriptParserOptions;

  constructor(options: ScriptParserOptions = {}) {
  ...
  parseScript(script: string): ParsedScript {
    try {
      ...
      throw new Error(`Script parsing failed: ${(error as Error).message}`);
    }
  }
  ...
  private parseNestedParameters(operation: string, paramsStr: string): ScriptParams {
    ...
    try {
      ...
      console.warn('Fallback to simple parsing:', (error as Error).message);
      return this.simpleParse(paramsStr);
    }
  }

  private processNestedStructures(str: string): string {
    return str
      // Handle ObjectId
  ...
  private parseChainedMethods(script: string): ChainedOperations {
    const chainedOps: ChainedOperations = {};
  ...
  private formatParametersByOperation(operation: string, parsed: any): ScriptParams {
    switch (operation) {
      case 'find':
  ...
  private simpleParse(str: string): ScriptParams {
    // Fallback simple parsing
    return { query: {} };
  }

  // Convert API parameters back to MongoDB script (for logging/debugging)
  parametersToScript(collection: string, operation: string, params: ScriptParams): string {
  ...
  validateScript(script: string): ScriptValidationResult {
    ...
    if (!this.options.allowDangerousOperators) {
      // Check for dangerous operators
      for (const op of dangerousOperators) {
        if (script.includes(op)) {
          return {
            valid: false,
            error: `Dangerous operator '${op}' is not allowed`
          };
        }
      }
  ...
  extractCollections(script: string): string[] {
  ...
  estimateComplexity(script: string): number {
    ...
    return Math.min(complexity, maxComplexity); // Cap at maxComplexity
  }

  // Convert ObjectId strings to ObjectId instances for MongoDB
  convertObjectIds<T = any>(obj: T): T {
    if (typeof obj === 'string' && ObjectId.isValid(obj)) {
      return new ObjectId(obj) as any;
    }
    
    if (Array.isArray(obj)) {
      return obj.map(item => this.convertObjectIds(item)) as any;
    }
  ...
  parseAndPrepare(script: string): ParsedScript {
